  ...
  class CountEverest {
    ...
    calculate() {
      ...
      const a = 31536e6,
        ...
        c = 36e5,
        o = {};
      (o.years = Math.floor(s / a)),
        (s %= a),
        (o.days = Math.floor(s / i)),
        (s %= i),
        (o.hours = Math.floor(s / c)),
        (s %= c),
        (o.minutes = Math.floor(s / 6e4)),
        (s %= 6e4),
        (o.seconds = Math.floor(s / 1e3)),
        (this.#e.classList.contains('ce-countdown--theme-10') ||
          (!this.#t.units.includes('years') && this.#t.units.includes('days'))) &&
          (o.days = Math.floor(n / i)),
    ...
    static theme9DrawCircles(e, t, s = '#284ED8') {
      ...
      const n = [
        { unit: 'days', max: 365, value: t.days, pad: !1 },
        { unit: 'hours', max: 24, value: t.hours, pad: !0 },
        { unit: 'minutes', max: 60, value: t.minutes, pad: !0 },
        { unit: 'seconds', max: 60, value: t.seconds, pad: !0 },
      ];
      n.forEach(({ unit: s, value: n, pad: a }) => {
        const i = e.querySelector(`.ce-${s}`);
        i && (i.textContent = a ? t.strPad(n, 2) : n);
      }),
        e.querySelectorAll('.ce-circle').forEach((e, t) => {
          ...
          a &&
            ...
            Array.from(e.querySelectorAll('[class*="ce-"]')).some((e) =>
              e.classList.contains(`ce-${i.unit}`)
            ) &&
            (function drawCircle(e, t, s, n) {
              ...
              const a = e.getContext('2d'),
                i = e.width / 2,
                c = e.height / 2,
                o = Math.min(i, c) - 0.05 * e.width,
                l = Math.max(3, 0.02 * e.width);
              a.clearRect(0, 0, e.width, e.height),
                (a.lineWidth = l),
                a.beginPath(),
                a.arc(i, c, o, 0, 2 * Math.PI),
                (a.strokeStyle = '#282828'),
                a.stroke(),
                a.beginPath(),
                (a.strokeStyle = n),
                a.arc(i, c, o, deg(0), deg((360 / s) * t)),
                a.stroke();
            })(a, i.value, i.max, s);
        });
    }
    static theme10FlipClock(e, t, s) {
      const n = { days: t.days, hours: t.hours, minutes: t.minutes, seconds: t.seconds };
      s &&
        ...
        Object.entries(n).forEach(([s, n]) => {
          ...
          if (!a) return;
          const i = a.querySelectorAll('.ce-digits').length,
            c = t.strPad(n.toString(), i, '0');
    ...
    static parseDataAttributes(e) {
      ...
      ['day', 'month', 'year', 'hour', 'minute', 'second'].forEach((s) => {
        ...
        [].forEach((s) => {
          const n = e.dataset[`ce${s.charAt(0).toUpperCase() + s.slice(1)}`];
          void 0 !== n && (t[s] = parseFloat(n));
        });
      const s = e.dataset.ceUnits;
      return s && (t.units = s.split(',').map((e) => e.trim())), t;
    }
