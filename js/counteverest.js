...
class CountEverest {
    ...
    wrapDigits(value) {
        return value.toString().padStart(2, '0').split('').map(digit => 
            `<span class="ce-digit">${digit}</span>`
        ).join('');
    }
    ...
    getLabel(unit, value) {
        ...
        return value === 1 && this.#settings.singularLabels ? this.#settings[singular] : this.#settings[plural];
    }
